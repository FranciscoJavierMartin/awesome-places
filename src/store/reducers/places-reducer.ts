import { IAction, IActionAddPlace, IActionSetPlaces } from './../../interfaces/action';
...
export default (
  ...
  switch (action.type) {
    case ADD_PLACE:
      const { id, title, image } = action.payload as IActionAddPlace;
      ...
      case ADD_PLACE:
        ...
        newState = {
          ...state,
          places: places.map((place: Place) => new Place(place.id, place.title, place.imageUri))
        }
        break;
    default:
      newState = state;
